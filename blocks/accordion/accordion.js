...
function createAccordionBlock(question, answer, image, uuid, parentElement, index, customUUID) {
  ...
  const summaryContent = label(
    ...
    span({ class: 'icon icon-minus w-6 h-6 absolute right-0 fill-current text-gray-400 rotate-0 transform transition-all ease-in-out minus [&_svg>use]:stroke-black' }),
  );
  if (image && index === 0) {
